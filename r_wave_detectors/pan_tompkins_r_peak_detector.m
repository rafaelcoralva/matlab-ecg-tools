function [qrs_amp_raw,qrs_i_raw,delay]=pan_tompkins_r_peak_detector(ecg,fs,gr)
%%%%%%%%%%%%%%% Hooman Sedghamiz 16/5/2017 %%%%%%%%%%%%%%%%%

% Complete implementation of the Pan-Tompkins R-wave detection algorithm

%% Inputs
% ecg - raw ecg signal
% fs - sampling frequency
% gr - flag to plot or not plot (set it 1 to plot or 0 to not plot)
...
% 1) The ECG signal is pre-processed: if the sampling frequency is high, it
% is downsampled and if it is low it is upsampled to make it 200Hz. The filter setup used is the same as that used in the
% Pan-Tompkins paper (a combination of low pass and high pass filter 5-15 Hz) to get rid of the
% baseline wander and muscle noise.

% 2) The filtered signal is differentiated using a differential filter to emphasize the QRS complex.

% 3) The signal is squared, accounting for negative QRS complexes and further emphasizing them.

% 4) The signal is averaged with a moving window (0.150 seconds in length) to get rid of noise.

% 5) Depending on the sampling frequency of the input signal, the filtering options are changed to best
% match the characteristics of your ecg signal.

% 6) The desicion rule of the Pan tompkins is implemented completely.
...
% complex. If however, an unreasonably long period has expired without an above-threshold
...
% rate and the second RR interval mean is the mean of the most regular RR intervals .
...
% f) In the final stage , the output of R waves detected in the smoothed signal is analyzed and double
% checked with the help of the output of the band-pass signal to improve
% detection and find the original index of the real R waves on the raw ecg signal.
...
qrs_amp =[]; % amplitude of R
qrs_i =[]; % index
SIG_LEV = 0;
nois_amp =[];
nois_i =[];
delay = 0;
skip = 0; % falg that becomes one when a T wave is detected
...
%% 1.Noise cancellation (Filtering between 5-15 Hz)
if fs == 200
    %% start figure
    ...
    Wn = 12*2/fs;
    N = 3; % order of 3
    [a,b] = butter(N,Wn,'low'); % lowpass filtering
    ...
    [a,b] = butter(N,Wn,'high'); % highpass filtering
...
else % if sampling frequency is not 200Hz...
...
%% 2. Derivative (gradient) filter H(z) = (1/8T)(-z^(-2) - 2z^(-1) + 2z + z^(2))
if fs ~= 200
    int_c = (5-1)/(fs*1/40);
...
%% 3. Squaring to enhance the dominant peaks
ecg_s = ecg_d.^2;
if gr
    ax(5)=subplot(325);plot(ecg_s);axis tight;title('Squared');
end

%% 4. Moving average Y(nt) = (1/N)[x(nT-(N - 1)T)+ x(nT - (N - 2)T)+...+x(nT)]
...
for i = 1 : length(pks)
    ...
    %% Update the heart_rate (two heart rate means one the most recent and the other is selected)
    if length(qrs_amp) >= 9
        ...
        if comp <= 0.92*mean_RR || comp >= 1.16*mean_RR
            ...
            m_selected_RR = mean_RR; % the latest regular beat's mean
        end
        
    end
    
    %% Calculate the mean of the last 8 R waves to make sure that QRS is not
    % missing(if no R detected , trigger a search back)
    
    if m_selected_RR
        test_m = m_selected_RR; % if the regular RR is availabe use it
    ...
    % Take a trace of thresholds of smoothed signal
    SIGL_buf = [SIGL_buf SIG_LEV];
    NOISL_buf = [NOISL_buf NOISE_LEV];
    THRS_buf = [THRS_buf THR_SIG];
    
    % Take a trace of thresholds of filtered signal
...
if gr
    hold on,scatter(qrs_i,qrs_amp,'m');
